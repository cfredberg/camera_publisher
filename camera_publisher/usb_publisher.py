...
class UsbCameraPublisher(Node):
    ...
    def __init__(self):
        ...
        if using_blue:
            ...
            if use_name: 
                gst_str = f'v4l2src device="/dev/v4l/{type_cam_src}/{camera_name}" ! videoconvert ! videoscale ! video/x-raw,width=320,height=240 ! appsink'
            else:
            ...
            gst_str = f'nvarguscamerasrc sensor-id={port} ! video/x-raw(memory:NVMM),width=1640,height=1232,framerate=30/1 ! nvvidconv ! video/x-raw(memory:NVMM),width=320,height=240 ! video/x-raw,format=BGRx ! videoconvert ! video/x-raw,format=BGR ! appsink'
